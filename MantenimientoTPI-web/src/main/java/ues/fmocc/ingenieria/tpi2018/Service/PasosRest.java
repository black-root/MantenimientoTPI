...
public class PasosRest implements Serializable{
   ...
   @Produces({MediaType.TEXT_PLAIN})
    ...
    public Response remove(@PathParam("id") Integer id) {
        if (ejbPasos.eliminar(ejbPasos.find(id))) {
    ...
    @Path("crea")
    public Response create(Pasos entity) {
        if (ejbPasos.crear(entity)) {
    ...
    @Path("editar")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response edit(Pasos entity) {
        if (ejbPasos.modificar(entity)) {
